...
def test_cli_requires_fetch_block_for_now(tmp_path: Path, capsys):
    """Pre-fetched-TSV path is a follow-up; for now, fetch block is required."""
    config = {
        "lead": {"variant_id": "1_1_A_T", "chromosome": "1", "position_bp": 1, "window_bp": 1000000},
        "exposure": {"trait_label": "x", "sumstats_path": "exposure.tsv"},
        "outcome": {"trait_label": "y", "fetch": {"source": "gwas_catalog", "accession": "Y"}},
    }
    config_path = tmp_path / "c.json"
    config_path.write_text(json.dumps(config))
    ...
    assert rc == 2
    captured = capsys.readouterr()
    assert "exposure.fetch" in captured.err
