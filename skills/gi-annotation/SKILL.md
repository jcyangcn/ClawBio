...
- **Always submitted async.** This skill sends `Prefer: respond-async` and polls, so don't expect a sync response from it — though the API itself serves annotation synchronously if you omit the header.
