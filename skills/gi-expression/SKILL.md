...
> These numbers (and the ~2.89 / ~0.4 strand pair below) still hold. The 2026-08-18 and 2026-08-19 API changes were request-validation and schema-publication changes only — no encoding or model change shipped — so every documented value here is unchanged and was deliberately not re-recorded.
...
- **A `tss_index` error reports at `loc: ["body"]`, never `body.tss_index`.** Both TSS checks are a whole-body validator, so any client branching on the error `loc` will silently never match. Match on `error.code` (`validation_failed`) and use `message` for display only — and read `error.details` defensively: it currently arrives as a bare array of FastAPI errors where the schema declares an object.
