...
- **Network calls only to public databases**: the API queries public TCGA data; no patient data is uploaded.
- **Disclaimer**: every `report.md` includes the standard ClawBio research-tool disclaimer.
- **Audit trail**: every run writes `reproducibility/commands.sh` with the exact curl commands and `reproducibility/run.json` with metadata.
