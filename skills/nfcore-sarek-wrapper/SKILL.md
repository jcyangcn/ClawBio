...
- **`split_fastq` uses the nf-core default (50 M reads/chunk) in normal runs, but `--demo` (`-profile test`) disables it — results can differ on tiny datasets.** nf-core/sarek 3.8.1 defaults `split_fastq = 50000000`; the upstream `test` profile overrides it to `0` (splitting off). The wrapper faithfully passes the pipeline default — it does **not** silently inject `0` — so a normal run and a `--demo` run of the *same* tiny dataset can legitimately produce different variant sets: FASTQ splitting adds a `fastp/` split-merge stage whose read-group handling can perturb calls on degenerate, very-small inputs (a known artifact of running production splitting on test-scale data, not a wrapper bug). This never matters at real WGS/WES depth. If you are reproducing a `-profile test` result, benchmarking against demo output, or running a deliberately small dataset, pass `--split-fastq 0` to match the test profile exactly. Do **not** change the default for production cohorts — splitting parallelises mapping and is beneficial at scale.
