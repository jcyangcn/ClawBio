...
# Issue #359: every maturity signal below used to be derived from Python file
# patterns, so a skill written in any other language could never register above
...
# generated as planned / spec-only / all-false / `demo_command: null`. Because
# the catalog is regenerated by a scheduled job, that also silently reverted a
# contributor's hand-corrected entry.
#
# Ordered by precedence, so a skill shipping more than one language resolves
# the same way on every run rather than flapping.
ENTRY_POINT_SUFFIXES = (".py", ".R", ".sh")

# How to invoke each. A `.R` entry point recorded as `python foo.R` is a
# documented command that cannot run.
INTERPRETERS = {".py": "python", ".R": "Rscript", ".sh": "bash"}

# Not entry points: package plumbing, and the thin API wrappers skills expose
# for the orchestrator rather than for a human.
NON_ENTRY_POINT_NAMES = {"__init__.py", "api.py", "conftest.py", "setup.py"}

TEST_FILE_GLOBS = ("test_*.py", "*_test.py", "run_test.sh", "test_*.R", "testthat.R")


def select_demo_script(skill_dir: Path, folder_name: str) -> Path | None:
    """Deprecated alias for `detect_entry_point`, kept for callers on main."""
    return detect_entry_point(skill_dir, folder_name)


def detect_entry_point(skill_dir: Path, folder_name: str) -> Path | None:
    """Return the skill's runnable entry point, whatever language it is in.

    Looks only at the skill root, never inside `tests/`, and prefers a file
    named after the folder so the choice is stable across runs.
    """
    def usable(f: Path, suffix: str) -> bool:
        if f.name in NON_ENTRY_POINT_NAMES or f.name.startswith("test_"):
            return False
        if f.name.endswith("_test" + suffix):
            return False
        parts = f.relative_to(skill_dir).parts
        # Never inside tests/, at any depth, and never build artefacts.
        return "tests" not in parts and "__pycache__" not in parts

    for suffix in ENTRY_POINT_SUFFIXES:
        # Root first, so a top-level script always wins; then nested, because
        # some skills keep theirs in `scripts/` (xena-tcga-gene-query). The old
        # rglob found those, and a root-only search silently demoted them.
        root = sorted(f for f in skill_dir.glob(f"*{suffix}") if usable(f, suffix))
        nested = sorted(
            f for f in skill_dir.rglob(f"*{suffix}")
            if usable(f, suffix) and f.parent != skill_dir
        )
        candidates = root + nested
        if not candidates:
            continue

        stem = folder_name.replace("-", "_")
        preferred = (
            f"{stem}{suffix}",
            f"{folder_name}{suffix}",
            f"cli{suffix}",
            f"__main__{suffix}",
        )
...
def detect_tests(skill_dir: Path) -> bool:
    """True if the skill ships a test file with something actually in it.

    Two defects here. A pytest-only glob made a shell harness invisible, and
    a bare `.exists()` glob granted the `tested` tier to a zero-byte file:
    my-awesome-skill (#345) shipped `tests/test_my_skill.py` as the canonical
    empty blob and the catalog advertised it as tested.
    """
...
def demo_invocation(script: Path | None, root: Path | None = None) -> str | None:
    ...
    if script is None:
        return None
    interpreter = INTERPRETERS.get(script.suffix, "python")
    base = root if root is not None else CLAWBIO_DIR
    try:
...
# Skills promoted to "mvp" status. This is a curatorial judgement, not an
# evidence-derived field, so it is hand-maintained.
#
# INVARIANT (issue #359): every name here must ship a runnable entry point.
# The set previously contained `claw-semantic-sim`, which has never shipped
# anything but a SKILL.md, so the catalog advertised `status: "mvp"` beside
# `maturity_tier: "spec-only"` in the same object and the README promised a
# `semantic_sim.py` that does not exist (#353). `drug-photo` was the same.
# `test_no_mvp_skill_lacks_an_entry_point` enforces the invariant; if it fails,
# remove the skill from this set rather than relaxing the test.
#
# Note the converse does NOT hold: 62 skills ship code and are deliberately
# not listed here, because "mvp" means curated and supported, not merely
# present. Do not turn this into `status = "mvp" if has_script`.
MVP_FOLDERS = {
    ...
    "genome-compare", "gwas-prs", "just-prs-mcp", "clinpgx", "gwas-lookup",
    "bigquery-public",
...
CHAINING: dict[str, list[str]] = {
    ...
    "just-prs-mcp": ["wgs-prs", "gwas-prs", "profile-report"],
...
def build_catalog() -> list[dict]:
    ...
    for skill_dir in sorted(SKILLS_DIR.iterdir()):
        ...
        # Entry point and tests, language-agnostic (issue #359)
        entry_point = detect_entry_point(skill_dir, folder_name)
        has_script = entry_point is not None
        has_tests = detect_tests(skill_dir)
